...
public class JavaMongoImpl implements JndiInterface {
  ...
  public StringBuffer customQueryDatatable(Map<String, Object> map) {
      ...
      */
      ...
      if (search.get("value") != null && !search.get("value").toString().equals("")) {
        for (String key : columns.keySet()) {
          complex.add(new BasicDBObject(columns.get(key).toString(),
              java.util.regex.Pattern.compile(search.get("value").toString())));
        }
      }
      ...
      if (columns.get(selected) != null) {
        if (complex.size() > 0) {
          // always '$or'
          cursor = coll.find(new BasicDBObject("$or", complex))
              .sort(new BasicDBObject(columns.get(selected).toString(), dir)).skip(nStart).limit(nLength);
        } else {
          cursor = coll.find().sort(new BasicDBObject(columns.get(selected).toString(), dir)).skip(nStart)
              .limit(nLength);
        }
      } else {
        if (complex.size() > 0) {
    ...
    sb.append(" ] , \"draw\" : " + map.get("draw") + " , \"recordsTotal\" : " + nTotal + " , \"recordsFiltered\" : "
        + nCount + " , \"status\" : \"OK\" , \"message\" : \"success\" }");
